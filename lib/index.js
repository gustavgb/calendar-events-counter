...
const { format, isSameWeek, addWeeks } = require('date-fns')
const HumanizeDuration = require('humanize-duration')

exports.countEvents = function (icalLink, name, weekOffset) {
  if (!icalLink || !name) {
    throw new Error('Invalid arguments')
  }

  const pattern = new RegExp(name, 'i')
  const now = addWeeks(new Date(), weekOffset || 0)
  return fetch(icalLink)
    ...
    .then(body => Object.entries(body)
      .filter(([, event]) =>
        ...
        isSameWeek(now, event.start, { weekStartsOn: 1 })
      )
      .map(([, event]) => ({
